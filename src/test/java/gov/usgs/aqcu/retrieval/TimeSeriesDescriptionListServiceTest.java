...
public class TimeSeriesDescriptionListServiceTest {
	...
	public void getRawResponseTest() throws Exception {
		...
		uniqueTimeseriesIdentifiers.add("ts-uid1");
		uniqueTimeseriesIdentifiers.add("ts-uid2");
		...
		List<TimeSeriesDescription> results = service.get(uniqueTimeseriesIdentifiers);
	...
	public void getTimeSeriesDescriptionListTest() {
		given(aquariusService.executePublishApiRequest(any(IReturn.class))).willReturn(new TimeSeriesDescriptionListByUniqueIdServiceResponse()
				.setTimeSeriesDescriptions(new ArrayList<TimeSeriesDescription>(Arrays.asList(DESC_1, DESC_2))));
		Map<String, TimeSeriesDescription> results = service.getTimeSeriesDescriptions("ts-uid1", "ts-uid2");
		assertEquals(results.size(), 2);
		assertEquals(DESC_1, results.get("ts-uid1"));
		assertEquals(DESC_2, results.get("ts-uid2"));
	}
